...
- iPhone 0.1.0, build **9**, revisión beta aprobada e `IN_BETA_TESTING` para Chefs. [TestFlight público](https://testflight.apple.com/join/kY83jmnk). ⚠️ **La app se cierra al abrirla desde la build 7 (16-09-2026)**: `useAuth.ts` cargaba React Native con un import dinámico y al recorrer sus exportaciones evaluaba el getter obsoleto `PushNotificationIOS`, cuyo `NativeEventEmitter` sin módulo nativo es fatal **solo en iOS** (Android no lo comprueba y por eso el APK funciona). Sentry lo registró 27 veces en 4 personas desde julio. Corregido con import nombrado y una prueba que lo impide; **falta compilar la build 10, enviarla a TestFlight y que alguien la abra**. El enlace y la build 9 están activos: el problema nunca fue el enlace.
