...
- iPhone 0.1.0, build **10** (EAS `a81037be-82e2-4de3-afa1-4571f0db617b`, Apple `f7d664bf-2130-4025-bd60-6eaed8c7f900`), procesada, en los grupos Equipo y Chefs e `IN_BETA_TESTING` desde el 16-09-2026. [TestFlight público](https://testflight.apple.com/join/kY83jmnk), el mismo enlace de siempre.
- Por qué existe la build 10: **desde la build 7 la app se cerraba al abrirla en iPhone**. `useAuth.ts` cargaba React Native con un import dinámico y, al recorrer sus exportaciones, evaluaba el getter obsoleto `PushNotificationIOS`, cuyo `NativeEventEmitter` sin módulo nativo es fatal **solo en iOS**; Android no hace esa comprobación, por eso el APK siempre funcionó. Sentry lo registró 27 veces en 4 personas desde julio. Corregido con import nombrado y una prueba que impide reintroducirlo. La build 9 sigue activa y **no debe repartirse**. Falta que alguien abra la build 10 en un iPhone: esa es la confirmación pendiente.
