...
const { db, guard, extract, blob, quota } = vi.hoisted(() => ({
  ...
  extract: { extractMenuStyleFromImage: vi.fn() },
...
vi.mock("@/lib/pdf/style-extract", () => ({
  extractMenuStyleFromImage: extract.extractMenuStyleFromImage,
}));
...
beforeEach(() => {
  ...
  extract.extractMenuStyleFromImage.mockReset().mockResolvedValue(SPEC);
...
describe("POST /api/restaurant/menu-style/from-image", () => {
  ...
  it("magic bytes malos → 415 sin gastar cuota ni llamar al extractor", async () => {
    ...
    expect(extract.extractMenuStyleFromImage).not.toHaveBeenCalled();
  ...
  it("extractor falla → 422 con code recipe_extraction_failed y no persiste", async () => {
    extract.extractMenuStyleFromImage.mockRejectedValue(new Error("foto ilegible"));
    const res = await postWithFile(JPEG_BYTES);
